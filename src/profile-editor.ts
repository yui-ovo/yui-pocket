...
export function editProfile(document: Document, host: ProfileHost, original: Person, options: {
  ...
  const save=button('保存联系人',()=>{if(busy)return;draft.name=name.input.value.trim();draft.remark=remark.input.value.trim();draft.description=description.input.value.trim();draft.account=account.input.value.trim();relationship();save.disabled=true;status.textContent='正在确认存档并保存…';
    const fields=scroll.querySelectorAll<HTMLInputElement|HTMLButtonElement|HTMLSelectElement>('input,button,select');fields.forEach(field=>field.disabled=true);
    void options.save(structuredClone(draft)).then(()=>{if(current())status.textContent='联系人已保存到本机';}).catch(error=>{if(current())status.textContent=error.message||'保存失败';}).finally(()=>{if(current()){save.disabled=false;fields.forEach(field=>field.disabled=false);}});});
