...
export function messageView(document: Document, sender: 'friend'|'self', text: string, identity: MessageIdentity, meta: string) {
  ...
  const copy=el('div','message-copy');copy.append(el('span','message-meta',meta),el('p','bubble',text));row.append(avatar,copy);return row;
}
