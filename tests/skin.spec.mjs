...
test('all embedded skin PNGs decode with real alpha and no external assets; artwork buttons operate',async({page})=>{
  ...
  const decoded=await page.locator('.phone').evaluate(async phone=>{
    const style=getComputedStyle(phone);const keys=['header','chat-header','white','pink','bubble-bow','bubble-wing','bubble-heart','row','frame','search','input','composer','date','bow','kitten','messages','contacts','mic','smile','send'];
  ...
  expect(decoded.find(x=>x.key==='bow').clear).toBeGreaterThan(100);
...
for(const [width,height] of [[375,740],[320,568],[375,340]])test(`raster skin ${width}x${height}: long bubbles grow while ornaments and controls stay fixed`,async({page})=>{
  ...
  expect(title.y).toBeGreaterThan(header.y+header.height*.5);expect(title.y+title.height).toBeLessThan(header.y+header.height-8);
  await expect(b(page,'聊天搜索（待接入）')).toBeDisabled();
  const designs=await page.locator('.chat-page .bubble').evaluateAll(nodes=>nodes.slice(0,2).map(el=>getComputedStyle(el,'::after').backgroundImage));expect(designs[0]).not.toEqual(designs[1]);
