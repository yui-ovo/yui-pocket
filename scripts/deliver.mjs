...
const bytes = await readFile('dist/yui.v0.2-step1.json');
validateScript(JSON.parse(bytes));
const destination = 'outputs/yui-v0.2-step1-source';
await mkdir(destination, { recursive: true });
const files = ['AGENTS.md', 'ROADMAP.md', 'PROGRESS.md', 'README.md', 'SOURCES.md', '.gitignore', 'package.json', 'package-lock.json', 'tsconfig.json', 'playwright.config.mjs', 'dist/yui.v0.2-step1.json', 'manifest.json', 'extension.js'];
...
await writeFile('outputs/yui.v0.2-step1.json', bytes);
for (const file of ['README.md', 'PROGRESS.md', 'SOURCES.md']) await copyFile(file, `outputs/${file}`);
const hash = createHash('sha256').update(bytes).digest('hex');
await writeFile('outputs/SHA256SUMS.txt', `${hash}  yui.v0.2-step1.json\n`);
