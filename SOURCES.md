...
## 2026-09-15 · 气泡去尾补丁（alpha.6）

根据用户圈图 codex-clipboard-1a3d921e-920a-4de6-a2f1-6653902b0980.png，只处理已获准公开的 bubble-bow-body.png 与 bubble-wing-body.png：沿用原图平滑上半轮廓，向下镜像替换带尾巴的底框部分。保持 PNG 尺寸、透明边缘和原图纹理；蝴蝶结、翅膀与小爱心为独立图层，未修改。像素处理记录更新在 skin/catalog.json；没有新增来源、网络请求或业务功能。

同轮后续参考图 codex-clipboard-559a30fa-60fc-4a8b-b7fc-66c77310a31a.png 用于消息头像/气泡底边关系；仅在现有隔离CSS中调整对齐与时间位置，没有复制参考图头像、消息内容或工具代码。新增检查验证长消息和头像大小变化时底边仍对齐。

## 2026-09-15 · 居中爱心顶栏（alpha.7）

用户用 codex-clipboard-f5a050d5-75cf-4071-98c6-e34c62df0d17.png 指定已供第二张UI素材左上角顶栏。header-centered.png从该原素材裁取[10,0,571,144]，保留心形、翅膀、缎带、蕾丝和控制符号；仅把Messages文字区域[225,88,334,118]以同一粉色空白区像素补齐，动态文字放回该处。未生成新画面，不使用截图中的红圈或黑背景。聊天搜索禁用并明确标注待接入；未增加网络/媒体/真实消息操作。先前两张供图与衍生裁片的公开发布授权继续适用。


## 2026-09-16 · 用户银白星星UI素材（alpha.8）

用户明确要求把现有皮肤换成此次附带的两张透明PNG：`codex-clipboard-b2242b1e-b5b0-422b-bdcb-7e9d4b7f6309.png`（1146×1373）和`codex-clipboard-20912177-75e2-4c24-87c3-f1703a6636ad.png`（1159×1356）。用户在看过alpha.8预览后明确回复“好好好我给你公开授权”，允许本轮两张银白素材裁片和内嵌扩展公开发布到yui-ovo/yui-pocket。不包含运行时私人头像或聊天。

`src/assets/skin/silver-*.png`是从这两张图直接裁取的银色栏框、背景、气泡、星形、爱心及按钮。来源和像素框逐项记录在catalog.json。用同高度的原图空白像素条去掉示例文字/人像/状态，不生成或替换成矢量；气泡用干净的上方轮廓镜像补齐下方以移除小尾巴，白色右角用干净左角镜像，星形固定叠加。按钮裁片做抗锯齿圆形透明裁切，信息按钮的示例粉色状态点用干净上沿镜像补齐。旧皮肤素材保留在源码，但不再被新皮肤CSS内嵌。

不使用供图中的转账卡片、撤回/已读/输入中/在线状态或示例人物；列表、名字和消息文本来自现有UI。当前无发送箭头原图，因此沿用真实演示发送按钮并换成银色，保持可用功能与素材装饰分离。全部在构建时嵌入，不增加运行时图片联网、依赖或宿主API。


## 2026-09-16 · 三主题与按钮修订（alpha.9）
